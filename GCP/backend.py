...
def possible_items_five_images(image_name):
    list_of_possible_items = []
    # splitImages outputs image for im.show
    (im1, im2, im3, im4, im5) = splitImages(image_name)
    ...
    # possible_items_one_image function takes in .jpg.
    list_1 = possible_items_one_image(image1)
...
def modify_list(my_list):
    ...
    for i in range(len(split_words_list)):
        ...
        split_words_list.append(
            "Poultry Meat") if split_words_list[i] == "Chicken" else None
        # add "others"
        split_words_list.append(
            "Other Vegetables") if split_words_list[i] == "Vegetable" else None
        split_words_list.append(
            "Other Fruit") if split_words_list[i] == "Fruit" else None
        split_words_list.append(
            "Other Pulses") if split_words_list[i] == "Pulses" else None
        # add plurals
        split_words_list.append(pluralize(split_words_list[i]))
        split_words_list.append(
            "Berries") if split_words_list[i] == "Berry" else None
        # add foods combined on csv file
        split_words_list.append(
            "Berries & Grapes") if split_words_list[i] == "Berries" or split_words_list[i] == "Grapes" else None
        split_words_list.append(
            "Lamb & Mutton") if split_words_list[i] == "Lamb" or split_words_list[i] == "Mutton" else None
        split_words_list.append(
            "Onions & Leeks") if split_words_list[i] == "Onions" or split_words_list[i] == "Leeks" else None
        split_words_list.append(
            "Wheat & Rye") if split_words_list[i] == "Wheat" or split_words_list[i] == "Rye" else None
...
def create_dictionaries1():
    ...
    fake_dict_from_GHG_csv = pd.read_csv('food_GHG.csv', usecols=[
                                         "Entity", "GHG emissions per kilogram (Poore & Nemecek, 2018)"], index_col=0).to_dict()
    dict_from_GHG_csv = fake_dict_from_GHG_csv[
        "GHG emissions per kilogram (Poore & Nemecek, 2018)"]

    fake_dict_from_land_csv = pd.read_csv('food_land.csv', usecols=[
                                          "Entity", "Land use per kilogram (Poore & Nemecek, 2018)"], index_col=0).to_dict()
    dict_from_land_csv = fake_dict_from_land_csv[
        "Land use per kilogram (Poore & Nemecek, 2018)"]

    fake_dict_from_water_csv = pd.read_csv('food_water.csv', usecols=[
                                           "Entity", "Freshwater withdrawals per kilogram (Poore & Nemecek, 2018)"], index_col=0).to_dict()
    dict_from_water_csv = fake_dict_from_water_csv[
...
def create_dictionaries2():
    dict_from_environmental = pd.read_csv('big_environmental.csv', usecols=[
                                          "Entity", "Emissions per kilogram", "Land use per kilogram", "Water withdrawals per kilogram"], index_col=3).to_dict()
...
def create_list_matching_database_with_possible_items(list_of_possible_items1):
    # remove leading and trailing whitespace from strings in list
    list_of_possible_items = modify_list(list_of_possible_items1)
    ...
    for x in list_of_possible_items:
        if x in dict_for_GHG:
            my_dict = {
                "name": x, "CO2": dict_for_GHG[x], "land": dict_for_land[x], "water": dict_for_water[x]}
